...
- $A \in \mathbb{R}^{n \times n}$ est la matrice des coefficients.
- $B \in \mathbb{R}^{n \times m}$ est la matrice d'entrée.
...
Ceci nous amène, à l'itération $k$, à devoir résoudre le problème aux moindres carrés linéaires :
...
::: {.callout-important}
A l'itération $k$, la fonction à minimiser dépend de $\Delta \theta$, la valeur du paramètre $\theta$ est fixée à $\theta_k$.
:::
...
### Solution du problème à l'itération $k$
...
$$
   A_k = \sum_{i=1}^{N} {J_{i,k}}^T J_{i,k} \in \mathrm{M}_{l, l}(\mathbb{R})
   \quad \text{et} \quad 
...
2. **Itération** : Pour chaque itération $k$ :
...
Développer à la main la première itération de l'algorithme de Gauss-Newton.
...
| Itération | $\theta_1$ | $\theta_2$ | Écart $\Delta \theta_1$ | Écart $\Delta \theta_2$ | Valeur du coût $F(\theta)$ |
|-----------|-------------|-------------|--------------------------|--------------------------|----------------------------|
| 0         | 1.000       | 1.000       | -                        | -                        | 0.500                     |
